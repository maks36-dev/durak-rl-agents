from src.deep_cfr.utils import *
...
class DurakEnv:
    """
    Game 'Durak' (2 player), deck 36 cards. One action in step.
    """
    ...
    def __init__(self):
        ...
        # table_cards: список пар (att_card_id, def_card_id)
    ...
    def reset(self) -> Tuple[Tuple, float, bool]:
        ...
        # For simplicity, let player 0 always go.
        self.attacker = 0
    ...
    def step(self, action: Optional[int]) -> Tuple[Tuple, float, bool]:
        """Get action and change game status"""
        if self.done:
            return self.get_current_obs()
        ...
        if action is not None:
            if action not in self.hands[current_player]: # illegal turn
                return self.get_current_obs()
    ...
    def _check_end(self):
        ...
        c0 = len(self.hands[0])
        c1 = len(self.hands[1])
        if c0 == 0 and c1 == 0: # draw
            self.done = True
            self.payoff = 0
        elif c0 == 0: # player0 win
            self.done = True
            self.payoff = +1
        elif c1 == 0: # player1 win
            self.done = True
            self.payoff = -1
